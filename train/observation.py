...
class observation_adapter(object):
    ...
    def cache(self, env_obs):
        ...
        self.ego_id = ego.id
        self.buffer[self.timestep][ego.id] = {'state': np.concatenate([ego.position[:2], [ego.heading+np.pi/2, ego.speed]]).astype(np.float32)}

        neighbors = env_obs.neighborhood_vehicle_states
        for neighbor in neighbors:
            self.buffer[self.timestep][neighbor.id] = {'state': np.concatenate([neighbor.position[:2], [neighbor.heading+np.pi/2, neighbor.speed]]).astype(np.float32)}
