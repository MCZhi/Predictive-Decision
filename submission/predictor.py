...
class InteractionEncoder(nn.Module):
    ...
    def forward(self, actors, mask):
        ...
        actors = torch.stack([self.time_1(actors[:, i], mask[:, i]) for i in range(N)], dim=1)
        actors = torch.stack([self.agent_1(actors[:, :, t], mask[:, :, t]) for t in range(T)], dim=2)
        actors = torch.stack([self.time_2(actors[:, i], mask[:, i]) for i in range(N)], dim=1)
        actors = torch.stack([self.agent_2(actors[:, :, t], mask[:, :, t]) for t in range(T)], dim=2)
        actors = torch.stack([self.time_3(actors[:, i], mask[:, i]) for i in range(N)], dim=1)
        actors = torch.stack([self.agent_3(actors[:, :, t], mask[:, :, t]) for t in range(T)], dim=2)
        actors = actors[:, :, -1]
...
class Decoder(nn.Module):
    def __init__(self, modalities=3):
        ...
        decoder = nn.Sequential(nn.Dropout(0.1), nn.Linear(256, 128), nn.ELU(), nn.Linear(128, 30*2))
        self.decoders = nn.ModuleList([copy.deepcopy(decoder) for _ in range(modalities)])
        scorer = nn.Sequential(nn.Dropout(0.1), nn.Linear(384, 128), nn.ELU(), nn.Linear(128, 1))
        self.scorers = nn.ModuleList([copy.deepcopy(scorer) for _ in range(modalities)])
        self.encode = nn.Sequential(nn.Linear(60, 96), nn.ReLU(), nn.Linear(96, 128))
    ...
    def forward(self, hidden, state):
        ...
        for i in range(self.modalities):
            pred = self.decoders[i](hidden).view(-1, N, 30, 2)
            trajs.append(pred + state[:, :, None, :2])
            score = self.scorers[i](torch.cat([hidden, self.encode(pred.view(-1, N, 60).detach())], dim=-1))
